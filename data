...
class ArgoDataset(Dataset):
    ...
    def read_argo_data(self, idx):
        ...
        agt_ts = np.sort(np.unique(df['TIMESTAMP'].values)) #时间戳序列
        mapping = dict()
        for i, ts in enumerate(agt_ts):  #i= index；ts = agt_ts中的数据
            mapping[ts] = i

        trajs = np.concatenate((
            df.X.to_numpy().reshape(-1, 1),  #reshape(-1,1)转换成1列
            df.Y.to_numpy().reshape(-1, 1)), 1)  #np.concatenate((a,b),axis=1),axis=1表示沿着数组水平方向进行拼接
        
        steps = [mapping[x] for x in df['TIMESTAMP'].values]
        steps = np.asarray(steps, np.int64)

        objs = df.groupby(['TRACK_ID', 'OBJECT_TYPE']).groups  #分组
        keys = list(objs.keys())
        ...
        agt_traj = trajs[idcs] #angent车的轨迹
        agt_step = steps[idcs] #agent时间戳 形式为1-50int

        del keys[agt_idx]  #删除agent车序列
        ctx_trajs, ctx_steps = [], []
        for key in keys:
            idcs = objs[key]
        ...
        data = dict()
        data['city'] = city
        data['trajs'] = [agt_traj] + ctx_trajs  #anget轨迹和其他车辆轨迹
        data['steps'] = [agt_step] + ctx_steps  #anget时间戳和其他车辆时间戳
        print(data['steps'])
        # print('data['traj'] = ',data['trajs'])
        return data

    #获取障碍物数据特征
    def get_obj_feats(self, data):
        orig = data['trajs'][0][19].copy().astype(np.float32) #agent第2s数据
        
        #config["rot_aug"] = False  train=True
        if self.train and self.config['rot_aug']:
            theta = np.random.rand() * np.pi * 2.0  #np.random.randn():返回一个浮点数0-1之间
        else:
            pre = data['trajs'][0][18] - orig
            theta = np.pi - np.arctan2(pre[1], pre[0]) #两帧之间车辆位置转角
        ...
        for traj, step in zip(data['trajs'], data['steps']):
            ...
            future_mask = np.logical_and(step >= 20, step < 50)  #logical_and:返回逻辑后的bool值
            post_step = step[future_mask] - 20
            post_traj = traj[future_mask]
            gt_pred[post_step] = post_traj #减去前2s的真实轨迹
            has_pred[post_step] = 1
            
            obs_mask = step < 20
            step = step[obs_mask]
            traj = traj[obs_mask]
            idcs = step.argsort() #argsort():小到大的索引
            step = step[idcs]
            traj = traj[idcs]
            ...
            x_min, x_max, y_min, y_max = self.config['pred_range']
            if feat[-1, 0] < x_min or feat[-1, 0] > x_max or feat[-1, 1] < y_min or feat[-1, 1] > y_max: #坐标限制在车道内？
                continue
    ...
    def get_lane_graph(self, data):
        ...
        for lane_id in lane_ids:
            ...
            if x.max() < x_min or x.min() > x_max or y.max() < y_min or y.min() > y_max:
                ...
                """Getting  requires original centerline"""
                polygon = self.am.get_lane_segment_polygon(lane_id, data['city']) #get_lane_segment_polygon：使用平均车道宽度，基于中心线绘制车道多边形。
        ...
        for lane_id in lane_ids:
            ...
            num_segs = len(ctrln) - 1 #道路中心线点个数
            
            ctrs.append(np.asarray((ctrln[:-1] + ctrln[1:]) / 2.0, np.float32))  ##?????
            #Node Feature考虑了车道线的朝向(Orientation)，即：Vi_end-Vi_start
            feats.append(np.asarray(ctrln[1:] - ctrln[:-1], np.float32))         ##?????
            
            #获取车道的转向（左转、直行、右转、掉头）、是否红绿灯
            ...
            intersect.append(lane.is_intersection * np.ones(num_segs, np.float32))  #是否在路口
            
        #构建车道拓扑，predecessors、successors、Left neighbors、right neighbors
        node_idcs = []
        count = 0
        ...
        for i, lane_id in enumerate(lane_ids):
            lane = lanes[lane_id]  #lanes[lane_id] = lane
            idcs = node_idcs[i]
        ...
        graph['turn'] = np.concatenate(turn, 0)  #车道转向反向
        graph['control'] = np.concatenate(control, 0) #交通信号灯
        graph['intersect'] = np.concatenate(intersect, 0) #十字路口
        graph['pre'] = [pre]
        graph['suc'] = [suc]
