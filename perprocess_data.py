...
# from data_huawei import HuaweiDataset as Dataset, from_numpy, ref_copy, collate_fn
from data_huawei_attrack import HuaweiDataset as Dataset, from_numpy, ref_copy, collate_fn
...
root_path = os.path.dirname(os.path.abspath(__file__)) #返回当前文件的绝对路径
sys.path.insert(0, root_path) #将当前文件的路径（root_path）插入到Python解释器的搜索路径（sys.path）的最前面
...
def train(config):
    # Data loader for training set     Raw Dataset
...
#     root_path, "dataset/train/data")
    ...
    # stores = [None for x in range(133441)]
    # stores = [None for x in range(19062)]
    # stores = [None for x in range(7925)]
    # stores = [None for x in range(17575//2)]
    stores = [None for x in range(9164)]
    t = time.time()
    #tqdm：快速、可扩展的进度条工具包
    ...
    data_loader = DataLoader(
        ...
        drop_last=False)
    #
    modify(config, data_loader,config["preprocess_train_single_small_curve"])
...
def test(config):
    ...
    # stores = [None for x in range(7925)]
    stores = [None for x in range(79250)]
...
def modify(config, data_loader, save):
    ...
    pickle.dump(store, f, protocol=pickle.HIGHEST_PROTOCOL)  #保存数据
    f.close()
...
class PreprocessDataset():
    ...
    def __getitem__(self, idx):
        from data_huawei import from_numpy, ref_copy

        data = self.split[idx]
        graph = dict()
        # for key in ['lane_idcs', 'ctrs', 'pre_pairs', 'suc_pairs', 'left_pairs', 'right_pairs', 'feats']:
        for key in ['ctrs','feats','centerlines']:
...
def worker_init_fn(pid):
    ...
    random_seed = np.random.randint(2 ** 64 - 1)
