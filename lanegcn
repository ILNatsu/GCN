...
config["lr_epochs"] = [32]
config["lr_func"] = StepLR(config["lr"], config["lr_epochs"])  #用于调整学习率的调度器。在训练过程中按照预先设定的步骤来调整学习率
...
config['preprocess_test'] = os.path.join(root_path, "dataset",'preprocess', 'test_test_huawei_2.p')
...
class MapNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        n_map = config["n_map"]  #config["n_actor"] = 128 config["n_map"] = 128
        norm = "GN"
        ng = 1
